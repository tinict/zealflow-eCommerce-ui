    ...
    >
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="h-[24px] overflow-hidden"><small>{mostRecentMes.message}</small></p>
        </div>
      </div>
      <div className="flex flex-col gap-1 items-end">
        <small>{mostRecentMes.time}</small>
        {mostRecentMes.status === 2 && mostRecentMes.user !== 1 && <div className="w-2 h-2 bg-red-400 rounded-full"></div>}
      </div>
    </div>
  );
}
